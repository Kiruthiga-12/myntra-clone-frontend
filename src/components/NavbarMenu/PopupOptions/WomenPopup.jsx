...
const WomenPopup = (props) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_subcategory_cat?category=Women`)
            .then((data) => {
                if (data.data.length > 0) {
                    setSubCat(data.data.slice())
                    data.data.map((li) => {
                        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_productcategory_category?category=Women&subcategory=${li.subcategory}`)
                            .then((data) => {
                                if (data.data.length > 0)
    ...
    return (
        <>
            {loader == true ? <Loader /> : <>
                {props.status === true &&
                    <Box sx={{
                        display: 'flex', justifyContent: 'space-evenly', marginLeft: '-170px', marginTop: '730px', width: '84%', height: '79vh',
...
export default connect(null, mapDispatchToProps)(WomenPopup);
