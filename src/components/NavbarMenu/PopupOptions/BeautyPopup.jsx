...
const BeautyPopup = (props) => {
    const [subcat, setSubCat] = useState([])
    const [prodcat, setProductCat] = useState([])
    const [loader, setLoader] = useState(true);
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_subcategory_cat?category=Beauty`)
            .then((data) => {
                if (data.data.length > 0) {
                    setSubCat(data.data.slice())
                    data.data.map((li) => {
                        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_productcategory_category?category=Beauty&subcategory=${li.subcategory}`)
                            .then((data) => {
                                if (data.data.length > 0)
    ...
    return (
        <>
            {loader == true ? <Loader /> : <>
                {props.status === true &&
                    <Box sx={{
                        display: 'flex', justifyContent: 'space-evenly', marginLeft: '-460px', marginTop: '730px', width: '78%', height: '79vh',
...
export default connect(null, mapDispatchToProps)(BeautyPopup);
