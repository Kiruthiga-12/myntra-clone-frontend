...
const MenPopup = (props) => {
    const [subcat, setSubCat] = useState([])
    const [prodcat, setProductCat] = useState([])
    const [loader, setLoader] = useState(true)
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_subcategory_cat?category=Men`)
            .then((data) => {
                if (data.data.length > 0) {
                    setSubCat(data.data.slice())
                    data.data.map((li) => {
                        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_productcategory_category?category=Men&subcategory=${li.subcategory}`)
                            .then((data) => {
                                (data.data.length > 0) ? setProductCat(data.data.slice()) : setProductCat([])
                            })
                    })
                }
                else
    ...
    return (
        <>
            {loader == true ? <Loader /> : <>
                {props.status === true &&
                    <Box sx={{
                        display: 'flex', justifyContent: 'space-evenly', marginLeft: '-60px', marginTop: '730px', width: '87%', height: '79vh',
...
export default connect(null, mapDispatchToProps)(MenPopup);
