...
const Pending_Orders = (props) => {
    ...
    useEffect(() => {
        const innerloop = axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_order?pending=pending`);
        const outerloop = axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_unique_order?pending=pending`)
            ...
            {loader == true ? <Loader /> : <>
                        ...
                        <Box sx={{ width: "98%", backgroundColor: 'white', margin: '10px', border: '1px solid grey', borderRadius: '10px' }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: "center", padding: '15px' }}>
                                ...
                                {li.order_status == 'packed' && <>
                                    <Select id={`pending_${index}_${li.order_id}`} sx={{ marginTop: "5px", flex: 2, marginLeft: "20px" }}
                                        variant='outlined' onChange={(e) => {
                                            setStatus(e.target.value);
                                            (e.target.value == 'shipped') ? setShippedDate(new Date()) : setShippedDate(0);
                                            (e.target.value == 'out for delivery') ? setOfdDate(new Date()) : setOfdDate(0);
                                            (e.target.value == 'delivered') ? setDeliveredDate(new Date()) : setDeliveredDate(0);
                                            (e.target.value == 'cancelled') ? setCancelledDate(new Date()) : setCancelledDate(0);
                                        }}>
                                        <MenuItem value='shipped' sx={{ color: 'black', backgroundColor: 'skyblue' }}>Shipped</MenuItem>
                                        <MenuItem value='out for delivery' sx={{ color: 'black', backgroundColor: 'skyblue' }}>Out For Delivery</MenuItem>
                                        <MenuItem value='delivered' sx={{ color: 'black', backgroundColor: 'skyblue' }}>Delivered</MenuItem>
                                        <MenuItem value='cancelled' sx={{ color: 'black', backgroundColor: 'skyblue' }}>Cancelled</MenuItem>
                                    </Select>
                                </>}
                            ...
                            <Button disabled={disab} sx={{ marginBottom: "10px", marginTop: "10px", marginLeft: "80%", padding: '5px 10px', color: 'white', backgroundColor: "rgb(7, 128, 97)", '&:hover': { backgroundColor: "rgb(7, 128, 97)" } }}
                                onClick={() => {
                                    setLoader(true);
                                    if (status == 'cancelled') {
                                        innerloop.map(async (li1) => {
                                            if (li.order_id == li1.order_id) {
                                                await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/update_item_stock_product?increase=${1}&product_id=${li1.product_id}`, {
                                                    'count': Number(li1.count) + 1
                                                })
                                                    .then((data) => { })
                                                await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/update_item_stock_cart?increase=${1}&product_id=${li1.product_id}`, {
                                                    'count': Number(li1.count) + 1
                                                })
                                                    .then((data) => { })
                                                await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/update_item_stock_wishlist?increase=${1}&product_id=${li1.product_id}`, {
                                                    'count': Number(li1.count) + 1
                                                })
                                                    .then((data) => { })
                                                await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/update_item_stock_order?increase=${1}&product_id=${li1.product_id}`, {
                                                    'count': Number(li1.count) + 1
                                                })
                                                    .then((data) => { })
                                                await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/update_item_stock_placeorder?increase=${1}&product_id=${li1.product_id}`, {
                                                    'count': Number(li1.count) + 1
                                                })
                                                    .then((data) => { })
                                            }
                ...
                {outerloop.length == 0 &&
                    <Typography variant='h5' sx={{
                        ...
                        fontFamily: 'verdana'
                    }}>No Order ID is in Pending state!!</Typography >}
            </>}
        </>
    )
}
...
export default connect(mapStateToProps, mapDispatchToProps)(Pending_Orders);