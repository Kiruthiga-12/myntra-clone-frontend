...
const Product_Edit_Details = (props) => {
    ...
    }, [comp_name, brand_name, catvalue, subcatvalue, productvalue, strikeprice, discount, price, color, descr, size, qty, prodkeywords,
        item_stock])
    ...
    useEffect(() => {
        ...
        const vendor = axios.get(`${process.env.REACT_APP_BACKEND_URL}/vendor_details?email=${props.email}`)
        const product = axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_product_product_id?product_id=${props.product}`)
        axios.all([brand, vendor, product])
            .then(axios.spread(function (branddet, vendordet, productdet) {
                (branddet.data.length > 0) ? setBrandName(branddet.data.slice()) : setBrandName();
                (vendordet.data[0].vendor_id != undefined) ? setVendorId(vendordet.data[0].vendor_id) : setVendorId()
                if (productdet.data.length > 0)
                    if (productdet.data[0].product_id != undefined) {
                        setProductID(productdet.data[0].product_id)
                        setCompanyName(productdet.data[0].comp_name);
                        setBrand_Name(productdet.data[0].brand_name);
                        setCatValue(productdet.data[0].category)
                        setSubCatValue(productdet.data[0].subcategory);
                        setProductValue(productdet.data[0].product);
                        setStrikePrice(Number(productdet.data[0].strike_price));
                        setDiscount(Number(productdet.data[0].discount));
                        setPrice(Number(productdet.data[0].price));
                        setColor(productdet.data[0].color);
                        setDescr(productdet.data[0].description);
                        setSize(productdet.data[0].size);
                        setQty(productdet.data[0].quantity);
                        setItemStock(productdet.data[0].count);
                        setProductKeywords(productdet.data[0].product_keywords);
                        setProductDate(new Date(productdet.data[0].product_date).toLocaleString('hi-EN').toLocaleUpperCase());
                        setFabric(productdet.data[0].fabric)
                        setPattern(productdet.data[0].pattern);
                        setNeck(productdet.data[0].neck);
                        setSleeveLength(productdet.data[0].sleeve_length);
                        setSizeFit(productdet.data[0].size_fit);
                        setOccasion(productdet.data[0].occasion);
                        setMainTrend(productdet.data[0].main_trend);
                        setWashCare(productdet.data[0].wash_care);
                        setClosure(productdet.data[0].closure)
                        setCompleteLook(productdet.data[0].complete_look)
                        setProductStatus(productdet.data[0].product_status);
                        setImage1(productdet.data[0].image1[0])
                        setImage2(productdet.data[0].image1[1])
                        setImage3(productdet.data[0].image1[2])
                        setImage4(productdet.data[0].image1[3])
                        setImage5(productdet.data[0].image1[4])
                        setImage6(productdet.data[0].image1[5])
                    }
                setLoader(false);
            }))
    }, [])
    ...
    return (<>
        {loader == true ? <Loader /> : <>
            ...
            <Box sx={{ paddingTop: '20px', paddingLeft: '20px', marginLeft: '20px', marginTop: '100px', backgroundColor: 'white', width: '90%', marginRight: '40px' }}>
                ...
                <br></br>
                <img src={`data:image/png;base64,${image1}`} alt='loading' width='120px' height='100px' id='prodimage1' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage1').style.display = 'none';
                ...
                <br></br>
                <img src={`data:image/png;base64,${image2}`} alt='loading' width='120px' height='100px' id='prodimage2' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage2').style.display = 'none';
                ...
                <br></br>
                <img src={`data:image/png;base64,${image3}`} alt='loading' width='120px' height='100px' id='prodimage3' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage3').style.display = 'none';
                ...
                <br></br>
                <img src={`data:image/png;base64,${image4}`} alt='loading' width='120px' height='100px' id='prodimage4' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage4').style.display = 'none';
                ...
                <br></br>
                <img src={`data:image/png;base64,${image5}`} alt='loading' width='120px' height='100px' id='prodimage5' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage5').style.display = 'none';
                ...
                <br></br>
                <img src={`data:image/png;base64,${image6}`} alt='loading' width='120px' height='100px' id='prodimage6' />
                <TextField variant='standard' type='file' sx={{ marginLeft: '30px', fontFamily: "cursive", fontWeight: "bold", fontSize: '18px' }}
                    onChange={async (e) => {
                        document.getElementById('prodimage6').style.display = 'none';
                ...
                <textarea style={{ padding: '5px 10px', marginTop: '20px', width: '500px', height: "100px", resize: 'vertical', maxHeight: "150px", outline: "none" }}
                    ...
                    id='product_edit_comments'
                    onBlur={() => {
                        if (reason.length > 3)
                            setDisable(false)
                    }} value={reason}></textarea>
                <span id='vendoraddproducts_error_message' style={{ marginLeft: '40px', color: "red", marginTop: '30px', fontFamily: 'TimesNewRoman', display: 'inline-block' }}> </span>
                <br></br>
                <Button sx={{
                        ...
                        async () => {
                            async function fun1() {
                                if (image1flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image1}`, 'product_edit1.png');
                                    if (file != undefined) {
                                        setImg1(file);
                                    }
                                }
                            ...
                            async function fun2() {
                                if (image2flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image2}`, 'product_edit2.png');
                                    if (file != undefined) {
                                        setImg2(file);
                                    }
                                }
                            ...
                            async function fun3() {
                                if (image3flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image3}`, 'product_edit3.png');
                                    if (file != undefined) {
                                        setImg3(file);
                                    }
                                }
                            ...
                            async function fun4() {
                                if (image4flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image4}`, 'product_edit4.png');
                                    if (file != undefined) {
                                        setImg4(file);
                                    }
                                }
                            ...
                            async function fun5() {
                                if (image5flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image5}`, 'product_edit5.png');
                                    if (file != undefined) {
                                        setImg5(file);
                                    }
                                }
                            ...
                            async function fun6() {
                                if (image6flag == false) {
                                    ...
                                    const file = dataURLtoFile(`data:image/png;base64,${image6}`, 'product_edit6.png');
                                    if (file != undefined) {
                                        setImg6(file);
                                    }
                                }
                            ...
                            function innerfunc() {
                                ...
                                formData.append('product_status', 'approved');
                                ...
                                axios.patch(`${process.env.REACT_APP_BACKEND_URL}/edit_product`, formData, {
                                    headers: {
                                    ...
                                    .then((data) => {
                                        if (data.data.modifiedCount == 1) {
                                            axios.post(`${process.env.REACT_APP_BACKEND_URL}/admin_product_approval`, {
...
export default Product_Edit_Details;