...
const ManageProfile = (props) => {
    const [name, setName] = useState('not entered');
    ...
    useEffect(() => {
        ...
        setEmail(props.admin_login);
        setNewMail(props.admin_login);
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/get_admin_add_det?email=${props.admin_login}`)
            .then((data) => {
                if (data.data.length > 0) {
                    setFlag(true)
                    setName(data.data[0].name);
                    setMob(data.data[0].mobile);
        ...
        {loader == true ? <Loader /> : <>
                    ...
                    <table style={{ margin: 'auto', marginTop: '50px', width: '600px' }}>
                        <tbody>
                            <tr >
                                ...
                                <td style={{ fontWeight: 'normal', fontFamily: 'verdana', fontSize: '18px', paddingLeft: '20px' }}>{name}</td>
                            </tr>
                            <tr >
            ...
            {editflag === true && <>
                <Box sx={{ marginTop: "30px", marginLeft: "30px", width: "900px", border: '1px solid black', boxShadow: '2px 2px 2px lightgrey,-2px -2px 2px lightgrey' }}>
                    ...
                    <TextField type='text' label='Full Name' value={newname} variant='outlined' sx={{ width: '85%', marginTop: '30px', marginLeft: '90px' }}
                    ...
                    <TextField type='text' label='Hint name' value={newhint} variant='outlined' sx={{ width: '85%', marginTop: '30px', marginLeft: '90px' }}
                        ...
                        onBlur={() => {
                            if (newname.length > 0 && newmob.length > 0 && newmail.length > 0 &&
                    ...
                    <Button disabled={disab} sx={{
                        ...
                        if (flag === false) {
                            axios.post(`${process.env.REACT_APP_BACKEND_URL}/add_adminadd_det`, {
                                'name': newname,
                                'mobile': newmob,
                                'email': newmail,
                                'gender': newgender,
                            ...
                            const edit_extra = axios.patch(`${process.env.REACT_APP_BACKEND_URL}/edit_adminadd_det?email=${email}`,
                                {
                                    'name': newname,
                                    'mobile': newmob,
                                    'email': newmail,
                            ...
                            axios.all([edit_extra])
                                .then(axios.spread(function (editdet) {
                                    if (editdet.data.modifiedCount != 0) {
                                        {
                                            toast.success('Details Edited Successfully!!', { autoClose: 3000 });
                                            setEditFlag(false)
                                        }
                                    }
                                    else {
