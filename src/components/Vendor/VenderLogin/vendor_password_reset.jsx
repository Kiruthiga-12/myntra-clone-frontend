...
const Vender_password_reset = (props) => {
    //status
    ...
    const vendor_reset = 'welcome to password reset page!'

    useEffect(() => {
        function pwd2() {
            if (logged) {
                setTargetLink('/partnerhome/loginandpwd');
            }
        }
        pwd2();
    }, [logged])
    useEffect(() => {
        toast.warning('welcome to password reset page!', {
            autoClose: 3000,
            position: toast.POSITION.BOTTOM_RIGHT,
            toastId: 'vendor-reset'
        })
        ...
        <>
            ...
            <Box sx={{
                    ...
                    onBlur={() => {
                        function pwd1() {
                            if (newpwd.length > 0 && newpwd.length == 8) {
                                ...
                                document.getElementById('value1').style.backgroundColor = 'rgb(248, 166, 166)';
                        ...
                        function pwd2() {
                            if (pwdpat2.test(newpwd)) {
                                ...
                                document.getElementById('value2').style.backgroundColor = 'rgb(248, 166, 166)';
                            }
                        }
                        function pwd3() {
                            if (pwdpat3.test(newpwd)) {
                                document.getElementById('value3').style.color = 'green';
                                document.getElementById('value3').style.backgroundColor = 'lightgreen';
                            }
                            else if (pwdpat3.test(newpwd) === false) {
                                document.getElementById('value3').style.color = 'red';
                                document.getElementById('value3').style.backgroundColor = 'rgb(248, 166, 166)';
                            }
                        }
                        function pwd4() {
                            if (pwdpat4.test(newpwd)) {
                                document.getElementById('value4').style.color = 'green';
                                document.getElementById('value4').style.backgroundColor = 'lightgreen';
                            }
                            else if (pwdpat4.test(newpwd) === false) {
                                document.getElementById('value4').style.color = 'red';
                                document.getElementById('value4').style.backgroundColor = 'rgb(248, 166, 166)';
                            }
                        }
                ...
                <Box sx={{ display: 'flex', alignItems: "center", justifyContent: 'space-between', marginLeft: '5px', marginRight: '5px', marginTop: '20px' }}>
                    <Typography id='value1' sx={{ flex: 3, borderRadius: '5px', border: '1px solid transparent', backgroundColor: "lightgrey", textAlign: "center", padding: '5px 5px' }}>8 Characters</Typography>
                    <Typography id='value2' sx={{ flex: 3, borderRadius: '5px', border: '1px solid transparent', backgroundColor: "lightgrey", textAlign: "center", padding: '5px 5px', marginLeft: '5px' }}>1 Uppercase </Typography>
                    <Typography id='value3' sx={{ flex: 3, borderRadius: '5px', border: '1px solid transparent', backgroundColor: "lightgrey", textAlign: "center", padding: '5px 5px', marginLeft: '5px' }}>1 Numeric</Typography>
                    <Typography id='value4' sx={{ flex: 3, borderRadius: '5px', border: '1px solid transparent', backgroundColor: "lightgrey", textAlign: "center", padding: '5px 5px', marginLeft: '5px' }}>1 Special</Typography>
                </Box>
                    ...
                    onBlur={() => {
                        function pwd1() {
                            if (confirmpwd.length == 0)
                                document.getElementById('vendor_resetpwd_errormsg').innerText = 'Please Enter password satisfies above criteria!!!';
                            else {
                                if (confirmpwd === newpwd)
                                    document.getElementById('vendor_resetpwd_errormsg').innerText = '';
                                else if (confirmpwd !== newpwd)
                                    document.getElementById('vendor_resetpwd_errormsg').innerText = 'New Password and confirm password doesn\' match';
                            }
                        }
                        function pwd2() {
                            setLoader(true);
                            axios.patch(`${process.env.REACT_APP_BACKEND_URL}/vendor_signup`, {
                                company_mailid: vemail,
                                vendor_password: newpwd
                            })
                                .then((data) => {
                ...
                <span id='vendor_resetpwd_errormsg' style={{ marginLeft: '40px', color: "red", marginTop: '30px', fontFamily: 'TimesNewRoman', display: 'inline-block' }}></span>
                <br></br>
                {loader == true ? <Loader /> : <>
...
export default connect(null, mapDispatchToProps)(Vender_password_reset);